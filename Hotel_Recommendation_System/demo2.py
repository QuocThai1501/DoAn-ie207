...
import time
from hotel_recommender import HotelRecommenderv2
...
# class khuyến nghị
recommender = HotelRecommenderv2(new_hotels, new_tourists)
#---------------------------------------------------------------------------------------------------------------------#
...
with st.sidebar:
    ...
    with st.form("search_form"):
        ...
        # Facilities
        st.subheader("Facilities and Requirements")
        facilities_options = [
        ...
        selected_facilities = st.multiselect(
            "Select preferred facilities and requirements",
            facilities_options,
        )
...
if 'search_params' in st.session_state:
    ...
    with st.expander("🔍 Your Search Criteria", expanded=True):
        ...
        # Chuẩn bị text cho facilities / requests
        facilities_text = ", ".join(params['facilities']) if params['facilities'] else "Không có"
        special_requests_text = params['special_requests'] if params['special_requests'] else "Không có"

        # Dòng 2: Facilities + Check-in
        cols2 = st.columns(2)
        cols2[0].markdown(f"**Facilities and Requirements:** {facilities_text}")
        cols2[1].markdown(f"**Check-in Range:** {checkin_text}")

        # Dòng 3: Special Requests + Check-out
        cols3 = st.columns(2)
        cols3[0].markdown(f"**Special Requests:** {special_requests_text}")
    ...
    if st.button("🔍 Search Hotels", use_container_width=True):
        with st.spinner(f"Finding best hotels in {params['location']}..."):
            ...
            facilities_input = params['facilities']
            if params['special_requests']:
                facilities_input.append(params['special_requests'])
                
            # Gọi hàm gợi ý
            recommendations = recommender.get_recommendations_tfidf_input(
                facilities_input=facilities_input,
